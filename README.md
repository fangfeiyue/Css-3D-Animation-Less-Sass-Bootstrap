...
但是会导致文件的体积增大，一方面图片本身的体积会变成之前的三分之四，另一方面本来图片是单独的文件，现在用base64将它放到的css样式中，从上面代码也能看出css里面增加了好多看不懂的代码。第二增加了浏览器解码的开销。第三不利于维护，因为很那从代码看出你这里到底放的是什么图片。所以base64只适合非常小的图片。
- 多分辨率适配
## 勘误
